...
<script type="text/javascript">
	function toSelectGoodsByType(pid,typeName){
		...
		$.ajax({
			url : "selectGoodsByPid.action?pid=" + pid,
			type : "get",
			dataType : "json",
					...
					if(i%2 == 1){
						...
						$(".interduce").append(
							"<div class='pic_1' style='height:85%'>"+
								"<div class='pic_img pic_00' style='background:url("+value.goods_pictureUrl+") no-repeat center center; background-size: cover;'>"+
	...
	<div class="top" id="top"
		...
		<form action="selectGoodsByName.action" method="post" name="selectGoods">
			<div class="searchCon">
				...
				<span id="sSpanGoods">巴登系列</span> <a class="btn" href="javascript:;"
					onclick="javascript:searchKey()">搜索</a> <input type="hidden" value=""
					id="search-form-id" size="35" name="search_goods_id">
			</div>
		</form>
	</div>
		...
		<div class="menu" oncopy="event.returnValue=false;"
			...
			<ul class="">
				<c:forEach items="${goodsTypes}" var="type">
					<li><a href="#"
						onclick="javascript:toSelectGoodsByType(${type.goods_id},'${type.goods_name }')">${type.goods_name }</a></li>
				</c:forEach>
			</ul>
		</div>
		<div class="goods">
					...
					<c:forEach items="${goodsList}" var="good" varStatus="status">
						<c:choose>
							<c:when test="${status.index%2 == 0}">
											<div class="pic_1" style='height:85%'>
												<div  class="pic_img pic_00" style="background:url('${good.goods_pictureUrl}') no-repeat center center; background-size: cover;">
